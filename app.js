    ...
    if (sectorData.length > 0) {
        ...
        sectors.forEach(sector => {
            const validReturns = sectorData
                ...
                .filter(val => val !== null && val !== undefined && val !== '' && val !== 0 && !isNaN(parseFloat(val)))
        ...
        console.log('最佳板块:', bestSector, '涨幅:', bestReturn);
        ...
        if (bestSector && bestReturn !== -Infinity && sectorElement && returnElement) {
            sectorElement.textContent = bestSector;
            returnElement.textContent = (bestReturn * 100).toFixed(2) + '%';
        } else {
            console.log('❌ 未找到有效的板块数据或DOM元素不存在');
            if (sectorElement) sectorElement.textContent = '暂无数据';
            if (returnElement) returnElement.textContent = '暂无数据';
        }
    } else {
...
function rtEstimation(aum, selectedBoards, rf, startDate = null, endDate = null) {
    ...
    ipoinfo_sub.forEach((item, index) => {
        if (item.offline_maxbuyamt !== null && item.offline_maxbuyamt !== undefined && item.offline_maxbuyamt !== '' &&
            item.pctchg !== null && item.pctchg !== undefined && item.pctchg !== '' &&
            item.lottery_b !== null && item.lottery_b !== undefined && item.lottery_b !== '') {
            ...
            // 确保转换后的数值是有效的
            if (!isNaN(offlineMaxbuyamt) && !isNaN(pctchg) && !isNaN(lotteryB)) {
    ...
    let iport = 0;
    let freecash_rt = 0;
    let tot_rt = rf; // 默认为无风险利率
    
    if (validCount > 0 && totalRtamt > 0 && delta > 0) {
        iport = (365 / delta) * (totalRtamt / aum);
        ...
        console.log('⚠️ 没有有效数据或时间跨度为0，使用默认值');
    }
    
    console.log('🎯 Python公式计算结果:', {
        delta天数: delta,
        有效记录数: validCount,
        '365/delta': delta > 0 ? (365/delta).toFixed(4) : 'N/A',
    ...
    return {
        ipoReturn: isNaN(iport) ? 0 : iport,
        freecashReturn: isNaN(freecash_rt) ? 0 : freecash_rt,
        totalReturn: isNaN(tot_rt) ? rf : tot_rt,
        validCount: validCount,
        totalRtamt: totalRtamt,
        startDate: startDate,
        endDate: endDate,
        deltaYears: delta > 0 ? delta / 365 : 0
    };
}
...
function updateComprehensiveStats() {
    ...
    const validReturns = rawData.filter(item => 
        item.pctchg !== null && item.pctchg !== undefined && item.pctchg !== '' && !isNaN(parseFloat(item.pctchg))
    );
    const avgReturn = validReturns.length > 0 
        ? validReturns.reduce((sum, item) => sum + parseFloat(item.pctchg), 0) / validReturns.length
        : 0;
    document.getElementById('avgFirstDayReturn').textContent = (avgReturn * 100).toFixed(2) + '%';
